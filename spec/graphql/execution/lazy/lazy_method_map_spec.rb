...
describe GraphQL::Execution::Lazy::LazyMethodMap do
  def self.test_lazy_method_map
    it "handles multithreaded access" do
      ...
      b = Class.new(a)
      c = Class.new(b)
      lazy_method_map.set(a, :a, :b)
      threads = 1000.times.map do |i|
        Thread.new {
          d = Class.new(c)
          assert_equal :a, lazy_method_map.get(d.new).value_method
          assert_equal :b, lazy_method_map.get(d.new).exec_method
        }
      end
      threads.map(&:join)
    end
    ...
    it "dups" do
      ...
      assert_equal :a, dup_map.get(a.new).value_method
      assert_equal :b, dup_map.get(a.new).exec_method
      assert_equal :a, dup_map.get(b.new).value_method
      assert_equal :b, dup_map.get(b.new).exec_method
      assert_equal :a, dup_map.get(c.new).value_method
      assert_equal :b, dup_map.get(c.new).exec_method
    end
  end
