...
if testing_rails?
  ...
  if jruby?
    ...
    ActiveRecord::Base.establish_connection(
      adapter: "postgresql",
      username: "postgres",
      password: ENV["POSTGRESQL_PASSWORD"], # empty in development, populated for GH Actions
      database: "graphql_ruby_test"
    )
    SequelDB = Sequel.connect("postgres://postgres:#{ENV["POSTGRESQL_PASSWORD"]}@localhost:5432/graphql_ruby_test")
  else
