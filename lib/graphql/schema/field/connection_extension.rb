  ...
  class Schema
    class Field
      class ConnectionExtension < GraphQL::Schema::FieldExtension
        ...
        def after_resolve(value:, object:, arguments:, context:, memo:)
          ...
          if object.is_a?(GraphQL::Schema::Object)
            context.query.after_lazy(value) do |resolved_value|
              wrap_if_needed(resolved_value, object.object, original_arguments, context)
            end
          else # Execution::Batching
            value.map.each_with_index do |collection, idx|
              parent_obj = object[idx]
              wrap_if_needed(collection, parent_obj, original_arguments, context)
            end
          end
        end
        ...
        def wrap_if_needed(value, object, original_arguments, context)
