      ...
      module Rules
        ...
        RESERVED_TYPE_NAME = ->(type) {
          if type.name.start_with?('__') && !type.introspection?
            ...
            GraphQL::Deprecated.warn("Name #{type.name.inspect} must not begin with \"__\", which is reserved by GraphQL introspection.")
            nil
          else
            # ok name
          end
        ...
        RESERVED_NAME = ->(named_thing) {
          if named_thing.name.start_with?('__')
            # TODO: make this a hard failure in a later version
            GraphQL::Deprecated.warn("Name #{named_thing.name.inspect} must not begin with \"__\", which is reserved by GraphQL introspection.")
            nil
          else
            # no worries
          end
