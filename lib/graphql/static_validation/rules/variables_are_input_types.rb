  ...
  module StaticValidation
    module VariablesAreInputTypes
      def on_variable_definition(node, parent)
        ...
        if type.nil?
          add_error("#{type_name} isn't a defined input type (on $#{node.name})", node, extensions: {
            "rule": "StaticValidation::VariablesAreInputTypes",
            "name": node.name,
            "type": type_name
          })
        elsif !type.kind.input?
          add_error("#{type.name} isn't a valid input type (on $#{node.name})", node, extensions: {
            "rule": "StaticValidation::VariablesAreInputTypes",
            "name": node.name,
            "type": type.name
          })
