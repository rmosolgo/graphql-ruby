...
module GraphQL
  module Execution
    class Runner
      ...
      def execute
        with_current_multiplex do
          ...
          trace.execute_multiplex(multiplex: @multiplex) do
            ...
            run_steps(trace)
            @multiplex.queries.map { |query| finish_query(query, results[query]) }
          end
      ...
      # @return [Hash] Partial results by query identity
      ...
      def begin_query(trace, query, results)
        if query.validate && !query.valid?
          results[query] = { "errors" => query.static_errors.map(&:to_h) }
          return
        end
