  ...
  module Backtrace
    ...
    def trace(key, metadata)
      ...
      when "execute_multiplex"
        clear_context_state
        pushed = true
        execution_context.push(metadata[:multiplex].queries)
      when "analyze_multiplex"
        pushed = true
        execution_context.push(metadata[:multiplex].queries)
      when "validate", "analyze_query", "execute_query", "execute_query_lazy"
        pushed = true
        execution_context.push(metadata[:query] || metadata[:queries])
      when "execute_field", "execute_field_lazy"
        pushed = true
        execution_context.push(metadata[:context])
      else
